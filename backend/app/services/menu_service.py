import asyncio
import json
import logging
from datetime import datetime, timezone
from decimal import Decimal
from uuid import UUID

from fastapi import HTTPException, status

from app.core.cache import redis_cache
from app.core.database import get_supabase_client
from app.models.menu_category import (
    MenuCategoryCreate,
    MenuCategoryResponse,
    MenuCategoryUpdate,
)
from app.models.menu_item import MenuItemCreate, MenuItemResponse, MenuItemUpdate

logger = logging.getLogger(__name__)

_MENU_CACHE_TTL = 86400  # 24 saat (saniye)

_CATEGORY_FIELDS = "id, branch_id, name, sort_order, is_active, created_at, updated_at"
_ITEM_FIELDS = "id, category_id, name, description, price, is_active, created_at, updated_at"


# ──────────────────────────────────────────────
# Cache helpers
# ──────────────────────────────────────────────


def _menu_cache_key(branch_id: UUID) -> str:
    """Hiyerarşik Redis key: menu:branch_{id}"""
    return f"menu:branch_{branch_id}"


async def _invalidate_branch_cache(branch_id: UUID) -> None:
    """
    İlgili şubenin menü cache'ini siler.
    Graceful degradation: Redis erişilemezse loglayıp devam eder, sistem çökmez.
    """
    key = _menu_cache_key(branch_id)
    try:
        await redis_cache.delete(key)
        logger.info("Cache invalidated for %s", key)
    except Exception as exc:
        logger.warning("Cache invalidation failed for %s (graceful degradation): %s", key, exc)


async def _resolve_branch_id_from_category(category_id: UUID) -> UUID:
    """Bir kategori ID'sinden branch_id'yi çözer."""
    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .select("branch_id")
        .eq("id", str(category_id))
        .is_("deleted_at", "null")
        .single()
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Category {category_id} not found.",
        )
    return UUID(resp.data["branch_id"])


# ──────────────────────────────────────────────
# Menu Categories — CRUD
# ──────────────────────────────────────────────


async def list_categories(branch_id: UUID) -> list[MenuCategoryResponse]:
    """Bir şubenin aktif kategorilerini sort_order'a göre döndürür."""
    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .select(_CATEGORY_FIELDS)
        .eq("branch_id", str(branch_id))
        .is_("deleted_at", "null")
        .order("sort_order")
        .execute()
    )
    return [MenuCategoryResponse(**row) for row in resp.data]


async def get_category(category_id: UUID) -> MenuCategoryResponse:
    """Tek bir kategoriyi getirir. Bulunamazsa 404."""
    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .select(_CATEGORY_FIELDS)
        .eq("id", str(category_id))
        .is_("deleted_at", "null")
        .maybe_single()
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Category {category_id} not found.",
        )
    return MenuCategoryResponse(**resp.data)


async def create_category(payload: MenuCategoryCreate) -> MenuCategoryResponse:
    """Yeni kategori oluşturur ve ilgili şubenin cache'ini invalidate eder."""
    client = get_supabase_client()

    # FK: şube var mı?
    branch_check = await asyncio.to_thread(
        lambda: client.table("branches")
        .select("id")
        .eq("id", str(payload.branch_id))
        .is_("deleted_at", "null")
        .maybe_single()
        .execute()
    )
    if not branch_check.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Branch {payload.branch_id} not found.",
        )

    insert_data = payload.model_dump()
    insert_data["branch_id"] = str(payload.branch_id)

    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories").insert(insert_data).execute()
    )
    result = MenuCategoryResponse(**resp.data[0])

    await _invalidate_branch_cache(payload.branch_id)
    return result


async def update_category(
    category_id: UUID, payload: MenuCategoryUpdate
) -> MenuCategoryResponse:
    """Kategoriyi günceller ve ilgili şubenin cache'ini invalidate eder."""
    update_data = payload.model_dump(exclude_none=True)
    if not update_data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No fields to update.",
        )

    branch_id = await _resolve_branch_id_from_category(category_id)

    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .update(update_data)
        .eq("id", str(category_id))
        .is_("deleted_at", "null")
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Category {category_id} not found.",
        )
    result = MenuCategoryResponse(**resp.data[0])

    await _invalidate_branch_cache(branch_id)
    return result


async def delete_category(category_id: UUID) -> None:
    """Soft delete + cache invalidation."""
    branch_id = await _resolve_branch_id_from_category(category_id)

    client = get_supabase_client()
    now = datetime.now(timezone.utc).isoformat()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .update({"deleted_at": now})
        .eq("id", str(category_id))
        .is_("deleted_at", "null")
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Category {category_id} not found.",
        )

    await _invalidate_branch_cache(branch_id)


# ──────────────────────────────────────────────
# Menu Items — CRUD
# ──────────────────────────────────────────────


async def list_items(category_id: UUID) -> list[MenuItemResponse]:
    """Bir kategorinin aktif ürünlerini döndürür."""
    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .select(_ITEM_FIELDS)
        .eq("category_id", str(category_id))
        .is_("deleted_at", "null")
        .order("created_at")
        .execute()
    )
    return [MenuItemResponse(**row) for row in resp.data]


async def get_item(item_id: UUID) -> MenuItemResponse:
    """Tek bir ürünü getirir. Bulunamazsa 404."""
    client = get_supabase_client()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .select(_ITEM_FIELDS)
        .eq("id", str(item_id))
        .is_("deleted_at", "null")
        .maybe_single()
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Menu item {item_id} not found.",
        )
    return MenuItemResponse(**resp.data)


async def create_item(payload: MenuItemCreate) -> MenuItemResponse:
    """Yeni ürün oluşturur ve ilgili şubenin cache'ini invalidate eder."""
    branch_id = await _resolve_branch_id_from_category(payload.category_id)

    client = get_supabase_client()
    insert_data = payload.model_dump()
    insert_data["category_id"] = str(payload.category_id)
    insert_data["price"] = str(payload.price)

    resp = await asyncio.to_thread(
        lambda: client.table("menu_items").insert(insert_data).execute()
    )
    result = MenuItemResponse(**resp.data[0])

    await _invalidate_branch_cache(branch_id)
    return result


async def update_item(item_id: UUID, payload: MenuItemUpdate) -> MenuItemResponse:
    """Ürünü günceller ve ilgili şubenin cache'ini invalidate eder."""
    update_data = payload.model_dump(exclude_none=True)
    if not update_data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No fields to update.",
        )

    # price → str (Supabase NUMERIC uyumu)
    if "price" in update_data:
        update_data["price"] = str(update_data["price"])

    # item → category → branch
    client = get_supabase_client()
    item_resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .select("category_id")
        .eq("id", str(item_id))
        .is_("deleted_at", "null")
        .single()
        .execute()
    )
    if not item_resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Menu item {item_id} not found.",
        )
    branch_id = await _resolve_branch_id_from_category(UUID(item_resp.data["category_id"]))

    resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .update(update_data)
        .eq("id", str(item_id))
        .is_("deleted_at", "null")
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Menu item {item_id} not found.",
        )
    result = MenuItemResponse(**resp.data[0])

    await _invalidate_branch_cache(branch_id)
    return result


async def delete_item(item_id: UUID) -> None:
    """Soft delete + cache invalidation."""
    client = get_supabase_client()
    item_resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .select("category_id")
        .eq("id", str(item_id))
        .is_("deleted_at", "null")
        .single()
        .execute()
    )
    if not item_resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Menu item {item_id} not found.",
        )
    branch_id = await _resolve_branch_id_from_category(UUID(item_resp.data["category_id"]))

    now = datetime.now(timezone.utc).isoformat()
    resp = await asyncio.to_thread(
        lambda: client.table("menu_items")
        .update({"deleted_at": now})
        .eq("id", str(item_id))
        .is_("deleted_at", "null")
        .execute()
    )
    if not resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Menu item {item_id} not found.",
        )

    await _invalidate_branch_cache(branch_id)


# ──────────────────────────────────────────────
# Public Menu — Cache-first read
# ──────────────────────────────────────────────


class _DecimalEncoder(json.JSONEncoder):
    """Decimal → str serialization for JSON."""

    def default(self, o: object) -> object:
        if isinstance(o, Decimal):
            return str(o)
        return super().default(o)


async def get_public_menu(branch_id: UUID) -> dict:
    """
    QR menü için cache-first okuma.
    1) Redis'te varsa → doğrudan döndür.
    2) Yoksa → DB'den çek, Redis'e yaz (TTL 24h), döndür.
    Graceful degradation: Redis erişilemezse sadece DB'den döner.
    """
    cache_key = _menu_cache_key(branch_id)

    # ── 1. Redis'ten oku ──
    try:
        cached = await redis_cache.get(cache_key)
        if cached is not None:
            logger.debug("Cache HIT for %s", cache_key)
            return json.loads(cached)
    except Exception as exc:
        logger.warning("Redis read failed for %s (fallback to DB): %s", cache_key, exc)

    # ── 2. Cache miss → DB'den çek ──
    logger.debug("Cache MISS for %s, fetching from DB", cache_key)

    # Şubeyi kontrol et
    client = get_supabase_client()
    branch_resp = await asyncio.to_thread(
        lambda: client.table("branches")
        .select("id, brand_id, name")
        .eq("id", str(branch_id))
        .eq("is_active", True)
        .is_("deleted_at", "null")
        .maybe_single()
        .execute()
    )
    if not branch_resp.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Branch {branch_id} not found or inactive.",
        )

    # Marka adını çek
    brand_resp = await asyncio.to_thread(
        lambda: client.table("brands")
        .select("name")
        .eq("id", branch_resp.data["brand_id"])
        .is_("deleted_at", "null")
        .maybe_single()
        .execute()
    )
    brand_name = brand_resp.data["name"] if brand_resp.data else ""

    # Kategoriler + ürünler
    cat_resp = await asyncio.to_thread(
        lambda: client.table("menu_categories")
        .select(_CATEGORY_FIELDS)
        .eq("branch_id", str(branch_id))
        .eq("is_active", True)
        .is_("deleted_at", "null")
        .order("sort_order")
        .execute()
    )

    categories = []
    for cat in cat_resp.data:
        cat_id = cat["id"]
        items_resp = await asyncio.to_thread(
            lambda cid=cat_id: client.table("menu_items")
            .select(_ITEM_FIELDS)
            .eq("category_id", cid)
            .eq("is_active", True)
            .is_("deleted_at", "null")
            .order("created_at")
            .execute()
        )
        categories.append({
            "id": cat["id"],
            "name": cat["name"],
            "sort_order": cat["sort_order"],
            "items": items_resp.data,
        })

    menu_data = {
        "branch_id": branch_resp.data["id"],
        "branch_name": branch_resp.data["name"],
        "brand_name": brand_name,
        "categories": categories,
    }

    # ── 3. Redis'e yaz (TTL 24h) ──
    try:
        await redis_cache.set(
            cache_key,
            json.dumps(menu_data, cls=_DecimalEncoder),
            ex=_MENU_CACHE_TTL,
        )
        logger.info("Cache SET for %s (TTL %ds)", cache_key, _MENU_CACHE_TTL)
    except Exception as exc:
        logger.warning("Redis write failed for %s (graceful degradation): %s", cache_key, exc)

    return menu_data
